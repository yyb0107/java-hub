 ...
 * @date 2019/4/8  23:02
 */
public class MiPay implements Pay {
    private double balance;

    public MiPay(String uid) {
    ...
    public PayResult pay(GoodsOrder order) {
        ...
        if (order.totalPrice < balance) {
            result = new PayResult("200", String.format("小米支付成功，信息如下：订单号：%s,总价格：%f", order.orderId, order.totalPrice), null);
